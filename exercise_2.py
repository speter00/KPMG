...
def queen_moves_num(n: int, r_q: int, c_q: int, obstacles: List[Tuple[int, int]]) -> int:
    ...
    row_obstacles = []  # obstacles on the queen's row
    col_obstacles = []  # obstacles on the queen's column
    desc_diagonal_obstacles = []  # obstacles on the same \ shaped diagonal as the queen
    asc_diagonal_obstacles = []  # obstacles on the same / shaped diagonal as the queen

    for obstacle in obstacles:
        if obstacle[0] - 1 == queen_pos[0]:  # row index match
            row_obstacles.append((obstacle[0] - 1, obstacle[1] - 1))
        if obstacle[1] - 1 == queen_pos[1]:  # col index match
            obstacle = (obstacle[0] - 1, obstacle[1] - 1)
            if obstacle in row_obstacles:  # if both row and column of obstacle matches the queen's, raise an error
                raise Exception("The obstacle can't be on the queen's field!")
            col_obstacles.append(obstacle)

        # obstacle is on the same \ shaped diagonal as the queen, if:
        # the sum of the coordinates matches
        if (obstacle[0] - 1) + (obstacle[1] - 1) == queen_cor_sum:
            desc_diagonal_obstacles.append((obstacle[0] - 1, obstacle[1] - 1))

        # obstacle is on the same / shaped diagonal as the queen, if:
        # the absolute difference of the coordinates matches...
        # and the sum of coordinates doesn't match
        elif abs((obstacle[0] - 1) - (obstacle[1] - 1)) == queen_cor_diff:
            asc_diagonal_obstacles.append((obstacle[0] - 1, obstacle[1] - 1))
